...
- `GET /binance/status` - Current bot status

### Trading Data
- `GET /binance/portfolio` - Current portfolio & positions
- `GET /binance/trades` - Trade history
- `GET /binance/market-data/{symbol}` - Market analysis
...
2. **When `/binance/start` is called**
